...
switch type
    ...
    case 'larva'
        model.nfids=4;model.D=model.nfids*2;model.name='larva';
    case 'mouse_paw'
        model.nfids=1;model.D=model.nfids*2;model.name='mouse_paw';
    case 'mouse_paw2'
        model.nfids=2;model.D=model.nfids*2;model.name='mouse_paw2';
    case 'mouse_paw3D'
        model.nfids=1;model.D=model.nfids*3;model.name='mouse_paw3D';
    case 'fly_RF2'
        model.nfids=6;model.D=model.nfids*3; model.name='fly_RF2';
    case 'mouse_paw_multi'
        model.nfids=6;model.D=model.nfids*3; model.name='mouse_paw_multi';
    otherwise
...
dfs={'type',5,'F',20,'radius',1,'nChn',3,'pids',[],'mask',[]};
[type,F,radius,nChn,pids,mask]=getPrmDflt(varargin,dfs,1);
F2=max(100,ceil(F*3));
xs=[];nfids=model.nfids;
if(type==5),
...
%     poscs = phis(:,1:nfids);
  ...
  if nfids == 1
    ...
    phis_left = project_points(X3d,zeros(3,1),zeros(3,1),Prm3D.fc_left,Prm3D.cc_left,Prm3D.kc_left);
    phis_right = project_points(X3d_right,zeros(3,1),zeros(3,1),Prm3D.fc_right,Prm3D.cc_right,Prm3D.kc_right);
...
if (strcmp(model.name,'mouse_paw3D'))
    dsAll = sqrt((del(:,1:nfids,:).^2) + (del(:,nfids+1:nfids*2,:).^2) + (del(:,2*nfids+1:nfids*3,:).^2));
else
    dsAll = sqrt((del(:,1:nfids,:).^2) + (del(:,nfids+1:nfids*2,:).^2));
end
dsAll = dsAll./distPup; ds=mean(dsAll,2);%2*sum(dsAll,2)/R;
if strcmp(model.name,'mouse_paw_multi'),
  ds = max(dsAll,[],2);
end
end
...
for n=1:N
    ...
    for l=1:L
        ...
        if strcmp(model.name,'mouse_paw3D')
            ...
            bbox=posInit(n,:);bbox(1:3)=bbox(1:3)+uncert;
        else
