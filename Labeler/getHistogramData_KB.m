function histDataStruct = getHistogramData_KB(hImage,ClimInit)
...
if isappdata(hImage, 'HistogramData')
    ...
    if histDataStruct.histRange(1) <= ClimInit(1) && histDataStruct.histRange(2) >= ClimInit(2),
      isset = true;
    end
end
if ~isset,
    [hrange, fbins, cnts, numbins] = computeHistogramData(hImage,ClimInit);
...
%==========================================================================
function [histRange, finalBins, counts, nbins] = computeHistogramData(hIm,ClimInit)
% This function does the actual computation

X = get(hIm,'CData');

xMin = min(min(X(:)),ClimInit(1));
xMax = max(max(X(:)),ClimInit(2));
origRange = xMax - xMin;
...
minX = calculateMinX(bins,finalBins,1:nbins);
maxX = calculateMaxX(bins,finalBins,1:nbins);
histRange = [minX maxX];
end
