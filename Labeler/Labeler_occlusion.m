function varargout = Labeler_occlusion(varargin)
% LABELER_OCCLUSION MATLAB code for Labeler_occlusion.fig
%      LABELER_OCCLUSION, by itself, creates a new LABELER_OCCLUSION or raises the existinglabeledpos
%      singleton*.
%
%      H = LABELER_OCCLUSION returns the handle to a new LABELER_OCCLUSION or the handle to
%      the existing singleton*.
%
%      LABELER_OCCLUSION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LABELER_OCCLUSION.M with the given input arguments.
%
%      LABELER_OCCLUSION('Property','Value',...) creates a new LABELER_OCCLUSION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Labeler_occlusion_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Labeler_occlusion_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Labeler_occlusion

% Last Modified by GUIDE v2.5 06-Oct-2014 17:04:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Labeler_occlusion_OpeningFcn, ...
                   'gui_OutputFcn',  @Labeler_occlusion_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Labeler_occlusion is made visible.
function Labeler_occlusion_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Labeler_occlusion (see VARARGIN)

% Choose default command line output for Labeler_occlusion
...
% UIWAIT makes Labeler_occlusion wait for user response (see UIRESUME)
...
while true,
  ...
  if keydown == 0 && strcmpi(get(handles.figure,'SelectionType'),'normal'),
    ...
    x = tmp(1,1);
    y = tmp(1,2);
    handles.hpoly(end+1) = plot(handles.axes_curr,x,y,'r+','MarkerSize',12);%,...
      %'KeyPressFcn',handles.keypressfcn);
    handles.htext(end+1) = text(x+10,y-10,num2str(numel(handles.htext)+1),'Color',[0 0.8 0.8],'Parent',handles.axes_curr);
    handles.template(end+1,:) = [x,y,0];
...
handles.labeledpos(i,1:2,handles.f) = pos;
...
if ~isfield(handles,'labeledpos') || size(handles.labeledpos,3) ~= handles.nframes,
  handles.labeledpos = nan([handles.npoints,3,handles.nframes]);
end
...
for i = 1:handles.npoints,
  if all(~isnan(handles.labeledpos(i,:,handles.f))) && handles.islocked(handles.f),
    if handles.labeledpos(i,3,handles.f)
        marker = 'o';
    else
        marker = '+';
    end
    set(handles.hpoly(i),'XData',handles.labeledpos(i,1,handles.f),...
      'YData',handles.labeledpos(i,2,handles.f),'Marker',marker);
  end
end
...
if handles.f > 1 && handles.islocked(handles.f-1),
  for i = 1:handles.npoints,
      if handles.labeledpos(i,3,handles.f-1)
          marker = 'o';
      else
          marker = '+';
      end
    set(handles.posprev(i),'XData',handles.labeledpos(i,1,handles.f-1),...
      'YData',handles.labeledpos(i,2,handles.f-1),'Marker',marker);
    set(handles.textprev(i),...
        'Position',[handles.labeledpos(i,1,handles.f-1)+10, handles.labeledpos(i,2,handles.f-1)-10])
  end
else
...
function varargout = Labeler_occlusion_OutputFcn(hObject, eventdata, handles) 
...
for i = 1:handles.npoints,
  handles.posprev(i) = plot(handles.axes_prev,nan,nan,'+','MarkerSize',8);%,...
    %'KeyPressFcn',handles.keypressfcn);
...
if handles.islocked(handles.f),
  ...
  handles.labeledpos(strcmp(get(handles.hpoly,'Marker'),'o'),3,handles.f) = 1;
  handles.labeledpos(strcmp(get(handles.hpoly,'Marker'),'+'),3,handles.f) = 0;
else
end
...
switch eventdata.Key,
  ...
  case 'o'
    old_state = get(handles.hpoly,'Marker');
    old_ocluded = strcmp(old_state,'o')';
    set(handles.hpoly(handles.pointselected & old_ocluded),'Marker','+')
    set(handles.hpoly(handles.pointselected & ~old_ocluded),'Marker','o')
    if handles.islocked(handles.f)
      handles.labeledpos(handles.pointselected,3,handles.f) = ~handles.labeledpos(handles.pointselected,3,handles.f);
    end
    guidata(hObject,handles);
end
...
if handles.pointselected(i),
  set(handles.hpoly(i),'LineWidth',2,'Color',[0 1 0]);
else
  set(handles.hpoly(i),'LineWidth',1,'Color',[1 0 0]);
end
...
s{end+1} = '* O toggles the point oclusion state for the selected points.';
